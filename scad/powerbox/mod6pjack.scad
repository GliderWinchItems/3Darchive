...
module j6mntholes(dia)
{
			ofy1 = 16;
			ofx1 = 7;
			translate([-25, ofy1,ofx1])
				rotate([0,90,0])
...
ofx = len+9-.25;
  difference()
  {
		union()
		{
			 translate([-ofx,-wid/2,-j6flr_b_thx])
				cube([len,wid,j6_ht+j6flr_b_thx],center=false);
		}
		union()
		{
			j6mntholes(3.5);
...
module j6mntblock_trim()
{
ofx = 3+9-.25;
	translate([-ofx-20,-20,-10])
...
/* Plate for base with self-tap holes */
module j6mntblock_plt()
{
ofx = 3+9-.25+4;
	difference()
	{
			translate([-ofx,0,(j6_ht+j6flr_b_thx)/2-j6flr_b_thx])
				cube([4,j6flr_b_wid + 8,j6_ht+j6flr_b_thx], center=true);
...
//translate([0,0,0]) j6mntblock_plt();

//translate([0,0,j6flr_b_thx]) total();
//translate([0,0,0]) total();
